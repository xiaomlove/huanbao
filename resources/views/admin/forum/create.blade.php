            ...
            <div class="card-block">
              <form class="form-horizontal" method="post" action="{{ route('forum.store') }}">
                ...
                <div class="form-group row{{$errors->has('slug') ? ' has-danger' : ''}}">
                  ...
                  <div class="col-sm-10">
                    <input id="" type="text" name="slug" value="{{ old('slug') }}" placeholder="slug" class="form-control form-control-success">
                    @if($errors->has('slug'))
                ...
                <div class="form-group row{{$errors->has('pid') ? ' has-danger' : ''}}">
                  ...
                  <div class="col-sm-10 select">
                    <select name="pid" class="form-control">
                      ...
                      <option value="99">不存在的</option>
                    </select>
                    @if($errors->has('pid'))
                    <small class="form-text">{{ $errors->first('pid') }}</small>
                    @endif
                  </div>
                </div>
