                    ...
                    <tr>
                        ...
                        <td>
                            @foreach($group as $item)
                                <label><input type="checkbox" value="{{ $item->name }}" name="permissions[]"{{ $role->hasPermissionTo($item->name) ? " checked" : "" }}>{{ $item->display_name }}</label>
                            @endforeach
                        </td>
                    </tr>
                @else
                    @foreach($group as $_key => $items)
                        ...
                        @else
                            <tr>
                                <th scope="row">{{ $displayNames[$key] ?? "" }}</th>
                                <td><label><input type="checkbox" value="{{ $items->name }}" name="permissions[]"{{ $role->hasPermissionTo($items->name) ? " checked" : "" }}>{{ $items->display_name }}</label></td>
                            </tr>
                        @endif
                    @endforeach
                @endif
